...
class LayerwiseMoEObserver:
    ...
    def __init__(
        ...
        Args:
            ...
            adapter: Model adapter used to locate the MoE module within a block
                and read num_experts/top_k/fused layout (replaces the old
                module_class_name_to_hook_regex / num_experts_attr_name fields).
        """
        self.model = model
        ...
        if self.adapter is None:
            raise RuntimeError(
                "LayerwiseMoEObserver requires an adapter (pass adapter=...)"
            )
    ...
    def _find_moe_module_in_block(self, block_idx: int) -> Optional[nn.Module]:
        """Find the MoE module within a transformer block via the adapter."""
        ...
        if not self.adapter.is_moe_layer(block):
            logger.debug(f"Block {block_idx} ({block_name}) has no MoE layer")
            self._moe_modules_cache[block_idx] = None
            return None

        module = self.adapter.get_moe(block)
        self._moe_modules_cache[block_idx] = module
        logger.debug(
            f"Found MoE module at {block_name}: {module.__class__.__name__}"
        )
        return module
    ...
    def _initialize_block_state(self, num_experts: int, hidden_dim: int | None = None) -> Dict[str, Any]:
        ...
        if not self.hook_config.record_pruning_metrics_only:
            if hidden_dim is None:
                raise ValueError(
                    "hidden_dim is required when record_pruning_metrics_only=False "
                    "(needed to allocate (E, H) merging-criteria trackers)"
                )
    ...
    def _process_moe_activations(
        ...
        """
        # Read MoE configuration from the adapter (layout-driven; replaces the
        # old num_experts_attr_name / top_k_attr_name hook-config fields).
        # get_layer_config expects the decoder *layer* (it resolves the MoE
        # block via adapter.get_moe(layer)), not the MoE module itself.
        layer_cfg = self.adapter.get_layer_config(self.blocks[block_idx], self.config)

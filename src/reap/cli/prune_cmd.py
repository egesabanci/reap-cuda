...
def prune_full(
    ...
    model_revision: opt.ModelRevision = None,
    local_files_only: opt.LocalFilesOnly = False,
    shuffle: opt.Shuffle = True,
    trust_observation_artifact: opt.TrustObservationArtifact = False,
    ...
    eval_tasks: opt.EvalTasks = None,
    eval_backend: opt.EvalBackend = "hf",
    eval_num_fewshot: opt.EvalNumFewshot = 0,
    eval_batch_size: opt.EvalBatchSize = 1,
    eval_limit: opt.EvalLimit = None,
    eval_baseline: opt.EvalBaseline = False,
    eval_data_path: opt.EvalDataPath = None,
    profile: Annotated[
        bool,
        typer.Option(
            "--profile/--no-profile",
...
def prune_layerwise(
    ...
    smoke_test: Annotated[
        ...
        typer.Option(
            "--smoke-test/--no-smoke-test",
            help="Generate a short sample before publishing the pruned checkpoint.",
            rich_help_panel="Run",
        ),
    ] = False,
    do_eval: Annotated[
